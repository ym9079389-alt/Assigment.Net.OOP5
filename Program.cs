    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            #region Practical
                ...
                IInsurable[] insurables = { standard};
                for (int i = 0; i < 1; i++)
                {
                ...
                Console.WriteLine($"Reference Assignment: {ReferenceEquals(standard, sh01)}");

                Shipment copy = standard.CopyShipment();
                Console.WriteLine($"CopyShipment: {ReferenceEquals(standard, copy)}");

                DeliveryUtilities.PrintSystemTitle("Shallow Copy");
                Shipment shallow = sh01.ShallowCopy();
                Console.WriteLine($"Original Address: {sh01.Destination.City}");
                Console.WriteLine($"Copy Address: {shallow.Destination.City}");

                Console.WriteLine("Changing copied shipment address");
                shallow.Destination.City = "Alex";

                Console.WriteLine($"Original Address: {sh01.Destination.City}");
                Console.WriteLine($"Copy Address: {shallow.Destination.City}");
                Console.WriteLine($"Same DeliveryAddress Object: {ReferenceEquals(sh01.Destination, shallow.Destination)}");


                sh01.Destination.City = city1;


                DeliveryUtilities.PrintSystemTitle("Deep Copy");
                Shipment deep = sh01.DeepCopy();
                Console.WriteLine($"Original Address: {sh01.Destination.City}");
                Console.WriteLine($"Copy Address: {deep.Destination.City}");

                Console.WriteLine("\nChanging copied shipment address...");
                deep.Destination.City = "Alex";
